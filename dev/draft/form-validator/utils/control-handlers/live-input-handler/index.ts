...
export const liveInputHandler: LiveInputHandler = (
   ...
   */
  const defaultValidateFunction = isTextControl
      ? validateWrittenData
      : validateClickedData,
    liveValidator =
      currentControl.customLiveValidator || defaultValidateFunction,
   ...
   */
  if (hasLiveValidator)
    setLiveValidatorResult(liveValidator, hooksData, controlOutputData);

  /**
   * Записать результаты дополнительного живого валидатора в объект вывода
   */
  if (hasAdditionalLiveValidator)
    setLiveValidatorResult(
      additionalLiveValidator,
      hooksData,
      controlOutputData,
    );
  ...
  if (!hasAnyError) {
     ...
     */
    if (hasControlHideErrorTimeout)
      currentControl._hideErrorTimeoutId &&
        clearTimeout(currentControl._hideErrorTimeoutId);
  }
